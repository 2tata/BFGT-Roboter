/*
BFGTmega32.h
header-Datei f√ºr die Benutzung des ¬µC-Boards "BFGTmega32"
des BZTG Oldenburg f√ºr das LCDisplay, die EEPROM-Nutzung,
die Nutzung der seriellen Schnittstelle, ...
am
Bildungszentrum f√ºr Technik und Gestaltung der Stadt Oldenburg
Standard: LC-Display an PORTB
*/

// Definitionen f√ºr Bitmanipulationen
#define setbit(PORT, bit)		(PORT|=	(1<<bit))   // Bit setzen
#define clearbit(PORT, bit)		(PORT&=~(1<<bit))	// Bit l√∂schen
#define togglebit(PORT, bit)	(PORT^=	(1<<bit))	// Bit invertieren

// Definitionen f√ºr LCDisplays, die kompatibel sind zu HD44780			// Pinning adapted to set interrupt pins free
...
void toggle_enable_pin(void)							// Unterst¸tzungsfunktion zum Invertieren von Bits
{
...
void uart_init(void)									// Initialisierung der seriellen Schnittstelle
{
	...
	UCSRC |= (1<<UCSZ1) | (1<<UCSZ0) | (1<<URSEL);	// L‰nge eines Zeichens auf 8 bit setzen
													// URSEL benˆtigt um auf UCSRC zuzugreifen
}

void uart_send_char(unsigned char c)					// Sendet einen einzelnen char
{
	while (!(UCSRA & (1<<UDRE)));	// Warten bis der UART zum Senden bereit
	UDR = c;						// Ñcì in Ausgangsregister schreiben
									// ‹bertragung wird automatisch gestartet
}
...
void uart_send_string(char *s)							// Senset einen String auf die serielle Schnittstelle
{
	while(*s)					//solange s != \0
	{
		...
		s++;					//weiter zum n‰chsten Zeichen
	}
}

int uart_get_int(void)									// Empf‰ngt ein integer
{
	while(bit_is_clear(UCSRA,RXC));	//warten auf Receive Complete
	return UDR;
}

void uart_send_int(unsigned int zahl, int sges)			// Sendet einen integer der Zeichenl‰nge sges 
{
...
{
	//Ausgabe einer Flieﬂkommazahl mit sges Gesamtstellen. 
...
{
	int z;
	uart_send_string("\n\n\r");	// 2x Zeilenumbruch, 1x R¸cklauf nach links
	uart_send_string("1)  Digitale Ausgaben an den Ports B und C\n\r");
	uart_send_string("2)  Taster als digitale Eing‰nge an Port A\n\r");
