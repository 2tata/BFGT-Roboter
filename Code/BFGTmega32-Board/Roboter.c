...
int waitRound = 0;				// Stepperdriver delay
int ulio = 0;					// IO für das Ultraschall Radar
int irio = 0;					// IO für die Infrarot sensoren
...
int Sleep_mode(void) // µC in Sleep mode setzen
{
	PORTC &= ~(1<<PC1);
...
int Sensor_steuerung(int input)
{
	if (input == 1) // Starte oder stoppe Ultraschall radar
	{
		if (ulio == 0)
		{
			ulio = 1;
	...
	if (input == 2) // Starte oder Stoppe Infrarot ausgabe
	{
		if (irio == 0)
		{
			irio = 1;
