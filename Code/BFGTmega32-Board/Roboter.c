...
int SensorRechts(void) // Messung Abstand Rechts
{
	...
	double d = ((1 - ((IR[0] * 0.0048828125) * (1.0 / 18.0))) / ((IR[0] * 0.0048828125) * (2.0 / 27.0)))*2;	// Distanz Umrechnung in cm für SHARP 2Y0A12 F 11
...
int SensorFront(void) // Messung Abstand Front
{
	...
	IR[3] = (ADCL+(256*ADCH));						// AD-Wandlung 10-Bit
	IR[0] = ((IR[1]+IR[2]+IR[3])/3);	// Mittlung der AD-Wandlung
	ADMUX &= ~(1 << MUX1);				// PC2 fuer AD wandlung AUS(2.0 / 27.0)
	double d = ((1 - ((IR[0] * 0.0048828125) * (1.0 / 18.0))) / ((IR[0] * 0.0048828125) * (2.0 / 14.0)))*2;	// Distanz Umrechnung in cm für SHARP 2D120XF 9Z
...
int SensorLinks(void) // Messung Abstand Links
{
	...
	double d = ((1 - ((IR[0] * 0.0048828125) * (1.0 / 18.0))) / ((IR[0] * 0.0048828125) * (2.0 / 12.0)))*2;	// Distanz Umrechnung in cm für SHARP 0A41SK F 33
	if (d < 5)						// Wenn d kleiner als 5 dann ...
	d = 400;						// Minimum 5cm, Meldung 400
	else if (d > 50)				// Wenn d größer als 50 dann ...
	d = 300;						// Maximum 50cm, Meldung 300
...
#define ZAELERTRIGGER 10
#define	ZAELERECHO 300
